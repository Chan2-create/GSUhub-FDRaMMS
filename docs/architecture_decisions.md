...
## 7. Firebase: `firebase_core` only, for now (carried forward)

Only `firebase_core` is a dependency. `firebase_auth`, `cloud_firestore`,
`firebase_storage`, `firebase_messaging` are deferred to 1.C (auth/DB/
storage/messaging configuration) rather than added speculatively now.

`lib/core/config/firebase_options.dart` is a placeholder with obviously
fake values, gitignored so a future `flutterfire configure` run's real
output is never committed. `firebase_options.example.dart` is the
committed template. `main.dart` checks `isPlaceholderFirebaseConfig` and
skips `Firebase.initializeApp` entirely while it's `true` — verified
working, not assumed. To connect a real project:
